...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT m FROM Message m WHERE m.conversationId = :convId AND m.id < :before AND m.deleted = false ORDER BY m.id DESC")
    ...
    @Query("SELECT m FROM Message m WHERE m.conversationId = :convId AND m.deleted = false ORDER BY m.id DESC")
    ...
    @Query("SELECT COUNT(m) FROM Message m WHERE m.conversationId = :convId AND m.id > :lastReadId AND m.senderId != :userId AND m.deleted = false")
    long countUnread(@Param("convId") Long convId, @Param("lastReadId") Long lastReadId, @Param("userId") Long userId);

    @Query("SELECT COUNT(m) FROM Message m WHERE m.conversationId = :convId AND m.senderId != :userId AND m.deleted = false")
