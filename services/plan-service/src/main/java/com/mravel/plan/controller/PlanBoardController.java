...
public class PlanBoardController {
    ...
    // labels
    @PostMapping("/labels")
    public ResponseEntity<ApiResponse<LabelDto>> upsertLabel(
            @PathVariable Long planId,
            @RequestBody LabelUpsertRequest req) {
        Long userId = currentUser.getId();
        LabelDto label = service.upsertLabel(planId, userId, req);
        return ResponseEntity.ok(
                ApiResponse.success("Cập nhật nhãn thành công", label));
    }
    ...
    public ResponseEntity<ApiResponse<?>> deleteLabel(
        ...
        return ResponseEntity.ok(
                ApiResponse.success("Xoá nhãn thành công", null));
    }
